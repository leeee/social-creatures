...
# From https://github.com/paykroyd/chats, with a few added changes
...
class ChatMessage(object):
    ...
    def __init__(self, message):
        ...
        try:
            self.timestamp = int(float(message.attributes['int:time-stamp'].value) / 1000)
        except KeyError:
            self.timestamp = 0;
    ...
    def __str__(self):
        return "(%s) %s to %s: %s" % (self.timestamp, self.sender, self.receiver, self.body)
...
def read_chats(directory):
    ...
    :param directory: directory to read chats fromi
